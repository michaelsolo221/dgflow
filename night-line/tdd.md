...
2. Track `_silence_count` in session state
3. If `<context>` detected: increment count
4. After 3 consecutive silences: return canned response from callback (skip LLM call)
5. Reset count when user activity detected (no `<context>` tag)
6. Without this handler: real calls get dead air after user goes silent
